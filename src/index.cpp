...
const oid index::checksum() {
  return oid(git_index_checksum(c_ptr_));
}
...
size_t index::size() const {
  return git_index_entrycount(c_ptr_);
}
...
bool index::has_conflicts() const {
  return git_index_has_conflicts(c_ptr_);
}
...
unsigned int index::version() const {
  return git_index_version(c_ptr_);
}
...
repository index::owner() const {
  return repository(git_index_owner(c_ptr_));
}

const git_index* index::c_ptr() const {
  return c_ptr_;
}
