...
size_t tree::size() const {
  return git_tree_entrycount(c_ptr_);
}
...
void tree::walk(
    traversal_mode mode,
    std::function<void(const std::string&, const tree::entry&)> visitor) const {
...
const git_tree* tree::c_ptr() const {
  return c_ptr_;
}
